        ...
        select case (flag_name)
            case ('BASINFORCINGFLAG')
                ...
                if (index(input_flag, ' met') > 0) then
                    ...
                    !> The following `if` clause checks whether the ` no_clim` substring is present in the `input_flag` string.
                    if (index(input_flag, ' no_clim') == 0) then
                        !> If substring does not exist, then recursively call the parse_basinforcingflag for each flag.
